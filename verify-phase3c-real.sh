...
if [ -z "$E2E_BASE_URL" ]; then
    echo "❌ E2E_BASE_URL not set. Example: export E2E_BASE_URL=https://app.zephix.yourdomain.com"
    exit 1
fi

if [ -z "$VITE_API_URL" ]; then
    echo "❌ VITE_API_URL not set. Example: export VITE_API_URL=https://api.zephix.yourdomain.com"
    exit 1
fi
