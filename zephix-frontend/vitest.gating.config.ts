/**
 * Vitest Gating Config — CI-blocking test suite
 *
 * These test files are verified stable and MUST pass on every PR.
 * If a test is flaky, fix it — do not move it to the legacy lane.
 *
 * RULES:
 *   - NEVER remove a test from gating. Only add.
 *   - CI enforces GATING_FILE_FLOOR. If include.length < floor, CI fails.
 *   - To move a legacy test into gating:
 *     1. Fix all failures in the test file
 *     2. Add its path to the `include` array below
 *     3. Remove it from test-health.json
 *     4. Update GATING_FILE_FLOOR to the new count
 *
 * Run: npm run test:gating
 */
import { defineConfig } from 'vitest/config';
import react from '@vitejs/plugin-react';
import path from 'path';

/**
 * Minimum number of test files in the gating suite.
 * CI will fail if include.length drops below this number.
 * Only increase this number — never decrease.
 */
export const GATING_FILE_FLOOR = 87;

const GATING_INCLUDES = [
      // ── UI Primitives ─────────────────────────────────────────
      'src/components/ui/__tests__/StatusBadge.test.tsx',
      'src/components/ui/button/__tests__/Button.test.tsx',
      'src/components/ui/form/__tests__/Checkbox.test.tsx',
      'src/components/ui/form/__tests__/FormField.test.tsx',
      'src/components/ui/form/__tests__/Select.test.tsx',
      'src/components/ui/input/__tests__/Input.test.tsx',
      'src/components/ui/overlay/__tests__/Modal.test.tsx',
      'src/components/ui/overlay/__tests__/Pagination.test.tsx',
      'src/components/ui/overlay/__tests__/Tabs.test.tsx',
      'src/components/ui/states/__tests__/EmptyStateCard.test.tsx',
      'src/components/ui/table/__tests__/DataTable.test.tsx',

      // ── Shell & System ────────────────────────────────────────
      'src/components/shell/__tests__/Sidebar.test.tsx',
      'src/components/shell/__tests__/SidebarResizeHandle.gating.test.tsx',
      'src/components/layouts/__tests__/DashboardLayout.sidebarResize.gating.test.tsx',
      'src/components/system/__tests__/ToastProvider.test.tsx',

      // ── Dashboard ─────────────────────────────────────────────
      'src/components/dashboard/__tests__/DashboardSidebar.test.tsx',
      'src/components/dashboard/__tests__/ProjectStats.test.tsx',
      'src/components/dashboard/__tests__/RecentProjects.test.tsx',
      'src/components/resources/__tests__/ResourceHeatmapCell.test.tsx',

      // ── Core Features ─────────────────────────────────────────
      'src/features/attachments/__tests__/attachments.test.tsx',
      'src/features/capacity/__tests__/capacity-gating.test.tsx',
      'src/features/explanations/__tests__/resolveExplanations.test.ts',
      'src/features/org-dashboard/__tests__/org-dashboard.gating.test.tsx',
      'src/features/org-dashboard/__tests__/org-dashboard.render.test.tsx',
      'src/features/portfolios/__tests__/portfolio-executive-summary.test.tsx',
      'src/features/scenarios/__tests__/scenario-gating.test.tsx',

      // ── Projects & Work Management ────────────────────────────
      'src/features/projects/__tests__/phase2c-guard-checks.test.tsx',
      'src/features/projects/components/__tests__/DuplicateProjectModal.test.tsx',
      'src/features/projects/components/__tests__/TaskListSection.restore.test.tsx',
      'src/features/projects/components/__tests__/TaskListSection.attributes.gating.test.tsx',
      'src/features/projects/components/__tests__/ProjectWorkToolbar.capabilities.gating.test.tsx',
      'src/features/projects/fields/__tests__/UnifiedWorkFieldsPanel.gating.test.tsx',
      'src/features/projects/tabs/__tests__/ProjectBudgetTab.test.tsx',
      'src/features/projects/tabs/__tests__/ProjectChangeRequestsTab.test.tsx',
      'src/features/projects/tabs/__tests__/ProjectResourcesTab.test.tsx',
      'src/features/projects/tabs/__tests__/ProjectTableTab.roleGating.test.tsx',
      'src/features/projects/tabs/__tests__/board-view.test.tsx',
      'src/components/views/__tests__/GanttChart.roleGating.test.tsx',
      'src/features/resources/utils/__tests__/allocation-errors.test.ts',
      'src/features/templates/utils/__tests__/order-preservation.test.ts',
      'src/features/templates/__tests__/tc-f1-template-browse.gating.test.ts',
      'src/features/templates/__tests__/tc-f2-instantiate-flow.gating.test.ts',
      'src/features/templates/__tests__/tc-f2b-stage2-fixes.gating.test.ts',
      'src/features/templates/__tests__/template-ux-1.gating.test.ts',
      'src/features/projects/components/__tests__/tc-f3-save-as-template.gating.test.ts',
      'src/pages/__tests__/mp-3-member-landing.gating.test.tsx',
      'src/features/work-management/__tests__/workTasks.stats.api.test.ts',

      // ── Layout & Routing ──────────────────────────────────────
      'src/layouts/__tests__/admin-nav.test.ts',

      // ── API & Utils ───────────────────────────────────────────
      'src/api/__tests__/mapArtifactApiError.test.ts',
      'src/api/__tests__/project-artifacts.mappers.test.ts',
      'src/features/projects/layout/__tests__/projectVisibleTabs.sprint52a.test.ts',
      'src/lib/__tests__/api.test.ts',
      'src/lib/api/__tests__/client.test.ts',
      'src/utils/__tests__/access.test.ts',
      'src/utils/access/__tests__/useEffectiveRole.taskCapabilities.test.tsx',

      // ── Artifacts (Sprint 5.2a) ─────────────────────────────────
      'src/features/artifacts/components/__tests__/CustomFieldRenderer.test.tsx',
      'src/features/artifacts/components/__tests__/ArtifactTypePickerModal.integration.test.tsx',
      'src/components/ui/__tests__/Rating.test.tsx',
      'src/hooks/__tests__/use-tree-roving-keydown.test.tsx',
      'src/hooks/__tests__/use-project-artifacts.optimistic.test.tsx',
      'src/features/workspaces/__tests__/SidebarWorkspaces.artifact-tree.invariants.test.tsx',
      'src/features/projects/layout/__tests__/stripLegacyVisibleTabs.test.ts',

      // ── Attributes (WAVE 1 Track A) ─────────────────────────────
      'src/features/attributes/__tests__/attributes.gating.test.tsx',
      'src/features/attributes/__tests__/field-ux.gating.test.tsx',

      // ── Work task types (WAVE 1 Track B) ─────────────────────────
      'src/features/work-management/components/__tests__/WorkTaskTypeBadge.gating.test.tsx',

      // ── Entity links (WAVE 1 Track D) ─────────────────────────────
      'src/features/entity-links/__tests__/entity-links.gating.test.tsx',

      // ── Theme UX (P-A1) ───────────────────────────────────────────
      'src/features/preferences/__tests__/theme-ux.gating.test.tsx',

      // ── Phase-Gate view (W2-B) ────────────────────────────────────
      'src/views/work-management/__tests__/projectPlan.w2b.gating.test.tsx',

      // ── Governance exceptions queue (W2-C) ────────────────────────
      'src/features/administration/__tests__/governance-exceptions.gating.test.tsx',

      // ── Governance policies admin (W2-F2) ─────────────────────────
      'src/features/administration/__tests__/governance-policies.gating.test.tsx',
      'src/features/administration/api/administration.snapshot-fallback.test.ts',

      // ── FE-GOV-1 governance console truth pass ────────────────────
      'src/features/administration/__tests__/fe-gov-1-truth-pass.gating.test.tsx',

      // ── OV-1 Phase A overview truth pass ─────────────────────────
      'src/features/projects/__tests__/ov-1-phase-a-truth-pass.gating.test.tsx',
      'src/features/projects/__tests__/ov-1-phase-b-overview.gating.test.tsx',
      'src/features/projects/__tests__/ov-1-phase-c-handoffs.gating.test.tsx',

      // ── GOV-BUILD WAVE1 Unit 2+5 gate reach ──────────────────────
      'src/features/projects/__tests__/gov-build-wave1-gate-reach.gating.test.ts',

      // ── SOD-RENDER-1 Unit 2 callerCanApprove affordance ───────────
      'src/features/phase-gates/__tests__/sod-render-1-caller-approve.gating.test.ts',

      // ── Admin landing containment (W2-F3) ───────────────────────
      'src/features/administration/__tests__/admin-landing.gating.test.tsx',

      // ── AUTH-1F admin reset-link UI ───────────────────────────────
      'src/features/administration/__tests__/auth-1f-reset-link.gating.test.tsx',

      // ── Inbox UX completion (PR-2) ───────────────────────────────
      'src/pages/__tests__/inbox-ux.gating.test.tsx',

      // ── Inbox filter URL sync (PR-2a) ────────────────────────────
      'src/pages/__tests__/inbox-filter-url.gating.test.tsx',

      // ── WM-B1 work surface quick hits ─────────────────────────────
      'src/features/projects/components/__tests__/wm-b1-work-surface.gating.test.tsx',
      'src/features/projects/tabs/__tests__/ProjectTableTab.taskIdDeepLink.gating.test.tsx',

      // ── CSV task import (W2-F1) ───────────────────────────────────
      'src/features/importer/__tests__/task-import.gating.test.tsx',

      // ── SESSION-FRONTEND-1 Item 2A — RequireWorkspace empty state ─
      'src/routes/__tests__/RequireWorkspace.gating.test.tsx',

      // ── SESSION-FRONTEND-1 Item 2B — Import/Export hang ───────────
      'src/features/integrations/__tests__/WorkspaceIntegrationsPage.gating.test.tsx',

      // ── Guardrails ────────────────────────────────────────────
      'src/test/guardrails/api-prefix.spec.ts',
];

// Runtime assertion — never deploy with fewer gating tests than the floor
if (GATING_INCLUDES.length < GATING_FILE_FLOOR) {
  throw new Error(
    `GATING VIOLATION: include has ${GATING_INCLUDES.length} files but floor is ${GATING_FILE_FLOOR}. ` +
    `Never remove tests from gating. Only add.`,
  );
}

export default defineConfig({
  plugins: [react()],
  resolve: {
    alias: {
      '@': path.resolve(__dirname, './src'),
    },
  },
  test: {
    environment: 'jsdom',
    setupFiles: ['./src/test/setup.ts'],
    globals: true,
    css: true,
    include: GATING_INCLUDES,
    env: {
      VITE_ATTRIBUTES_MOCK: 'true',
    },
  },
});
