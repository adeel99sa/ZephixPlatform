...
import {
  ...
  Edit, Trash2, Shield, Filter, CheckSquare, Square, Download, Upload
} from 'lucide-react';
...
export default function AdminUsersPage() {
  ...
  const handleRoleChange = async (userId: string, newRole: 'admin' | 'pm' | 'viewer') => {
                ...
                {filteredUsers.map((user) => (
                  <tr key={user.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <select
                        value={user.role}
                        onChange={(e) => handleRoleChange(user.id, e.target.value as 'admin' | 'pm' | 'viewer')}
          ...
          <div className="bg-white rounded-lg p-6 max-w-md w-full">
            ...
            <div className="space-y-3 mb-6">
              {(['viewer', 'pm', 'admin'] as const).map(role => (
                ...
                >
                  <div className="font-medium capitalize">{role === 'pm' ? 'Project Manager' : role}</div>
