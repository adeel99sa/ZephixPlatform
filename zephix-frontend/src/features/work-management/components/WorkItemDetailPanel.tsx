...
import { TaskRelationsSection } from '@/features/entity-links/components/TaskRelationsSection';
import { listEntityLinksForEntity } from '@/features/entity-links/entityLinks.api';
...
export function WorkItemDetailPanel({
  ...
  useEffect(() => {
    let cancelled = false;
    void listEntityLinksForEntity(workspaceId, 'TASK', taskId)
      .then((links) => {
        if (!cancelled) setRelationsCount(links.length);
      })
      .catch(() => {
        if (!cancelled) setRelationsCount(0);
      });
    return () => {
      cancelled = true;
    };
  ...
  const linksCount = relationsCount;
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {activeTab === 'links' && (
            <TaskRelationsSection
              ...
              taskId={taskId}
              canEdit={canEditWork}
              onLinksCountChange={setRelationsCount}
            />
          )}
        </div>
      </div>
