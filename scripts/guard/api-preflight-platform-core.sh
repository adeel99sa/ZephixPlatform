#!/usr/bin/env bash
# api-preflight-platform-core.sh
#
# Live preflight: authenticates against staging and exercises the core CRUD
# path (workspace → project → task). Requires STAGING_SMOKE_KEY and network
# access to staging. NOT a local contract shape check — see
# api-contract-platform-core.sh for the offline guard.
#
# Named *preflight* because it is a live probe, not a static validator.
...
echo "platform-core preflight verification passed"
