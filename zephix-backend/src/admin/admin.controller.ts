...
import { ListGovernancePoliciesQueryDto } from '../modules/governance/dto/list-governance-policies-query.dto';
import { CreateGovernancePolicyDto } from '../modules/governance/dto/create-governance-policy.dto';
...
export class AdminController {
  ...
  constructor(
    ...
    private readonly governancePoliciesService: GovernancePoliciesService,
  ...
  async listGovernancePolicies(
    @Request() req: AuthRequest,
    @Query() query: ListGovernancePoliciesQueryDto,
  ) {
    const { organizationId } = getAuthContext(req);
    const active = query.active ?? true;
    const policies = await this.governancePoliciesService.listPolicies(
      organizationId,
      query.scopeType,
      active,
    );
    const data = policies.map((p) => ({
      ...
      active: p.active,
      lockedComponents: p.lockedComponents,
      mandatoryBaselineTemplateId: p.mandatoryBaselineTemplateId,
  ...
  @ApiResponse({ status: 409, description: 'Overlapping policy exists' })
  async createGovernancePolicy(
    @Request() req: AuthRequest,
    @Body() dto: CreateGovernancePolicyDto,
  ) {
    const { organizationId } = getAuthContext(req);
    return this.governancePoliciesService.createPolicy(organizationId, dto);
  }
