...
// TODO: Re-add governance imports after governance workstream merges
...
export class AdminController {
  ...
  async updateUser(
    ...
    try {
      ...
      if (body.role) {
        // Map public API role names to legacy org service roles
        const legacyRole = body.role === 'member' ? 'pm' : body.role;
        await this.organizationsService.updateUserRole(
          organizationId,
          userId,
          legacyRole as 'admin' | 'pm' | 'viewer',
          currentUserId,
        );
      }
  ...
  // TODO: Re-add governance-policies endpoints after governance workstream merges
  // Removed: GET /governance-policies, POST /governance-policies
  // Both depend on GovernancePoliciesService (not yet on staging)
