...
describe('ResourceAllocationService - validateGovernance', () => {
  ...
  const mockResourceRepository = {
    findOne: jest.fn().mockResolvedValue({ id: 'resource-1', capacityHoursPerWeek: 40 }),
  };
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: getRepositoryToken(WorkTask),
          useValue: {},
        },
        {
  ...
  afterEach(() => {
    jest.clearAllMocks();
    mockResourceRepository.findOne.mockResolvedValue({ id: 'resource-1', capacityHoursPerWeek: 40 });
  });
