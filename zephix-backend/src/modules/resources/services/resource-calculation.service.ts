...
export class ResourceCalculationService {
  ...
  async calculateResourceImpact(taskId: string): Promise<number> {
    ...
    const resourceNames = task.assignedResources.split(',').map(r => r.trim());
    ...
    const weeksForTask = this.calculateWeeksBetween(task.startDate, task.endDate);
  ...
  async calculateTotalResourceLoad(resourceName: string, startDate: Date, endDate: Date): Promise<number> {
    ...
    const resourceTasks = tasks.filter(task => 
      ...
      task.assignedResources.toLowerCase().includes(resourceName.toLowerCase())
    );
