...
export class ProjectsViewService {
  ...
  async create(workspaceId: string, organizationId: string, dto: CreateProjectSimpleDto) {
    ...
    await this.viewRepo.save([
      ...
      this.viewRepo.create({ projectId: saved.id, type: 'gantt', label: 'Gantt', sortOrder: 3, isEnabled: false }),
  ...
  async listViews(projectId: string) {
    return this.viewRepo.find({ where: { projectId }, order: { sortOrder: 'ASC' } });
  }

  async enableView(projectId: string, type: string, isEnabled: boolean) {
    const view = await this.viewRepo.findOne({ where: { projectId, type: type as any } });
