...
export class SmokeKeyGuard implements CanActivate {
  canActivate(context: ExecutionContext): boolean {
    ...
    const runtimeHeader = String(request.headers['x-zephix-env'] || '').trim();
    ...
    if (runtimeHeader.toLowerCase() !== 'staging') {
      throw new ForbiddenException('x-zephix-env header must be staging');
    }
    if (!smokeKey) {
