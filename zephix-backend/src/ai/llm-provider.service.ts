...
export class LLMProviderService implements OnModuleInit {
  ...
  constructor(
    ...
    this.providerSettings = {
      ...
      model: this.configService.get<string>('anthropic.model') || 'claude-3-sonnet-20240229',
      dataRetentionOptOut: this.configService.get<boolean>('anthropic.dataRetentionOptOut') || false,
      enableDataCollection: this.configService.get<boolean>('anthropic.enableDataCollection') || false,
      enforceNoDataRetention: this.configService.get<boolean>('llm.enforceNoDataRetention') !== false, // Default true
      apiVersion: this.configService.get<string>('anthropic.apiVersion') || '2023-06-01',
    };
  }
  ...
  private async validateProviderSettings(): Promise<void> {
    ...
    if (this.providerSettings.enforceNoDataRetention) {
      if (!this.providerSettings.dataRetentionOptOut) {
        issues.push('Data retention opt-out not enabled (ANTHROPIC_DATA_RETENTION_OPT_OUT should be true)');
      }
      
      if (this.providerSettings.enableDataCollection) {
        issues.push('Data collection is enabled (ANTHROPIC_ENABLE_DATA_COLLECTION should be false)');
      }
    }
  ...
  private logProviderSettings(): void {
    ...
    this.logger.log(`   Data Retention Opt-Out: ${this.providerSettings.dataRetentionOptOut ? '✅ YES' : '❌ NO'}`);
    this.logger.log(`   Data Collection Enabled: ${this.providerSettings.enableDataCollection ? '❌ YES' : '✅ NO'}`);
    this.logger.log(`   Enforce No Data Retention: ${this.providerSettings.enforceNoDataRetention ? '✅ YES' : '❌ NO'}`);
    
    // Log compliance status
    const isCompliant = this.isDataRetentionCompliant();
    this.logger.log(`   🛡️  Data Retention Compliance: ${isCompliant ? '✅ COMPLIANT' : '❌ NON-COMPLIANT'}`);
    
    if (!isCompliant) {
      this.logger.warn('⚠️  WARNING: Current configuration may allow data retention for training');
    }
  }

  private isDataRetentionCompliant(): boolean {
    return this.providerSettings.dataRetentionOptOut && !this.providerSettings.enableDataCollection;
  }
  ...
  async sendRequest(request: LLMRequest): Promise<LLMResponse> {
    ...
    if (!this.isDataRetentionCompliant()) {
      this.logger.warn('⚠️  Making LLM request with non-compliant data retention settings');
    }
    ...
    try {
      ...
      const body = {
        ...
        max_tokens: request.maxTokens || this.configService.get<number>('anthropic.maxTokens'),
        messages: [
      ...
      this.logger.debug(`📤 Sending LLM request (compliance: ${this.isDataRetentionCompliant() ? 'OK' : 'WARNING'})`);
      ...
      if (!fetchResponse.ok) {
        ...
        this.logger.error(`LLM API request failed: ${fetchResponse.status} ${fetchResponse.statusText}`, errorData);
        throw new Error(`LLM API request failed: ${fetchResponse.status} ${fetchResponse.statusText}`);
      }
      ...
      this.metricsService.incrementLlmTokens(provider, model, 'input', llmResponse.usage.inputTokens);
      this.metricsService.incrementLlmTokens(provider, model, 'output', llmResponse.usage.outputTokens);
  ...
  getComplianceStatus(): {
    ...
    if (this.providerSettings.enableDataCollection) {
      ...
      recommendations.push('Set ANTHROPIC_ENABLE_DATA_COLLECTION=false or remove the variable');
    }

    if (!this.providerSettings.enforceNoDataRetention) {
      recommendations.push('Set LLM_ENFORCE_NO_DATA_RETENTION=true for strict compliance');
    }
