  ...
  implements MigrationInterface
{
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    if (!conflictsTable) {
      console.warn(
        'resource_conflicts table does not exist, skipping migration',
      );
      return;
    }

    // Add resolved_by_user_id column
    const resolvedByUserIdColumn = conflictsTable.findColumnByName(
      'resolved_by_user_id',
    );
    ...
    const resolutionNoteColumn =
      conflictsTable.findColumnByName('resolution_note');
