...
export class SafeProductionDatabaseSetup1756690611596 implements MigrationInterface {
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        for (const mapping of renameMappings) {
            if (tableNames.includes(mapping.old)) {
                await queryRunner.query(`ALTER TABLE "${mapping.old}" RENAME TO ${mapping.new}`);
                console.log(`Renamed ${mapping.old} to ${mapping.new}`);
            }
        }

        // 2. SAFE ALLOCATION TABLE CONSOLIDATION
        if (tableNames.includes('project_allocations') && tableNames.includes('resource_allocations')) {
            ...
            if (!existingColumns.includes('organization_id')) {
                await queryRunner.query(`ALTER TABLE resource_allocations ADD COLUMN organization_id UUID`);
            }
            if (!existingColumns.includes('user_id')) {
                await queryRunner.query(`ALTER TABLE resource_allocations ADD COLUMN user_id UUID`);
            }
        ...
        if (tableNames.includes('resource_allocations')) {
            ...
            if (columnNames.includes('organizationId') && columnNames.includes('organization_id')) {
