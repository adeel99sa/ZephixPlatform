...
export class WorkflowInstance {
  ...
  @ManyToOne(() => WorkflowTemplate, template => template.instances, { onDelete: 'CASCADE' })
  ...
  getCurrentStageEntry(): StageHistoryEntry | null {
    ...
    return this.stageHistory
      .filter(entry => entry.stageId === this.currentStage && !entry.exitedAt)
      .sort((a, b) => new Date(b.enteredAt).getTime() - new Date(a.enteredAt).getTime())[0] || null;
  }
  ...
  canProgressToNextStage(): boolean {
    ...
    const pendingApprovals = this.approvals.filter(
      approval => approval.stageId === this.currentStage && approval.status === 'pending'
    );
  ...
  getTotalDuration(): number | null {
    ...
    const lastEntry = this.stageHistory
      ...
      .sort((a, b) => new Date(b.exitedAt!).getTime() - new Date(a.exitedAt!).getTime())[0];
    
    if (!firstEntry || !lastEntry?.exitedAt) return null;
    
    return new Date(lastEntry.exitedAt).getTime() - new Date(firstEntry.enteredAt).getTime();
  }
  ...
  getStageMetrics(): Record<string, { duration: number; attempts: number }> {
    ...
    this.stageHistory.forEach(entry => {
      ...
      if (entry.exitedAt) {
        const duration = new Date(entry.exitedAt).getTime() - new Date(entry.enteredAt).getTime();
