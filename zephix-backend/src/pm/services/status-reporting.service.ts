...
export class StatusReportingService {
  ...
  async generateStatusReport(
    ...
    try {
      ...
      return {
        ...
        overallStatus: savedReport.overallStatus,
        healthScore: savedReport.healthScore,
        ...reportData,
      };
    } catch (error) {
  ...
  private async calculateHealthMetrics(projectId: string, performanceAnalysis: any): Promise<any> {
    ...
    const overallStatus = healthScore >= 80 ? 'green' : healthScore >= 60 ? 'yellow' : 'red';
