...
export class IntakeDesignerController {
  ...
  async generateFromDescription(
    ...
    try {
      ...
      this.logger.log(
        `✅ Form generated with ${(result.confidence * 100).toFixed(1)}% confidence`,
      );
    ...
    try {
      ...
      switch (intent.type) {
        case 'generate':
          ...
          const generationResult = await this.generateFromDescription(
            generateDto,
            orgId,
            user,
          );
        ...
        case 'refine':
          if (!context?.existingForm) {
            throw new BadRequestException(
              'No existing form context provided for refinement',
            );
          }
        ...
        default:
          return {
            message:
              'I can help you create intake forms using natural language. Try describing what kind of form you need, like: "Create a marketing project request form that captures project name, budget, timeline, and key stakeholders."',
            type: 'acknowledgment',
            suggestions: [
      ...
      return {
        message:
          'I encountered an issue processing your request. Could you please try rephrasing?',
        type: 'error',
      };
    }
  }
  ...
  async refineForm(
    ...
    try {
      ...
      throw new InternalServerErrorException(
        'Form refinement failed. Please try again.',
      );
    }
  }
  ...
  async deployForm(
    ...
    try {
      ...
      throw new InternalServerErrorException(
        'Form deployment failed. Please try again.',
      );
    }
  }
  ...
  async analyzeForm(
    ...
    try {
      ...
      const analysis = await this.performFormAnalysis(
        dto.formStructure,
        dto.analysisType,
      );
  ...
  async bulkGenerate(
    ...
    this.logger.log(
      `📊 Processing bulk form generation (${dto.requests.length} forms)`,
    );
  ...
  async submitFeedback(
    ...
    try {
      ...
      this.logger.log(
        `Feedback received: Rating ${dto.rating}/5, Would use again: ${dto.wouldUseAgain}`,
      );

      return {
        success: true,
        message:
          'Thank you for your feedback! It helps us improve the AI form generator.',
      };
    } catch (error) {
  ...
  @Get('templates')
  async getQuickTemplates(): Promise<
    ...
    return [
      {
        ...
        prompt:
          'Create a marketing project intake form that captures project name, campaign type, target audience, budget range, timeline, key deliverables, and stakeholder information.',
      },
      {
        name: 'IT Support Request',
        description: 'IT help desk and support ticket form',
        prompt:
          'Design an IT support request form with fields for issue type, priority level, affected systems, detailed description, business impact, and contact information.',
      },
      {
        name: 'Vendor Onboarding',
        description: 'New vendor registration and onboarding',
        prompt:
          'Build a vendor onboarding form that collects company information, services offered, insurance details, references, and compliance documentation.',
      },
      {
        name: 'Event Planning Request',
        description: 'Corporate event and meeting planning',
        prompt:
          'Create an event planning request form with event type, date preferences, attendee count, budget, catering needs, AV requirements, and special accommodations.',
      },
      {
        name: 'Training Request',
        description: 'Employee training and development requests',
        prompt:
          'Design a training request form that captures training topic, business justification, target audience, preferred format, timeline, and success metrics.',
      },
    ];
  }
    ...
    if (
      ...
      return {
        ...
        clarificationQuestion:
          "Could you describe what kind of intake form you'd like to create? For example, is this for project requests, support tickets, or something else?",
        suggestions: [
          'Project intake form',
  ...
  private async performFormAnalysis(
    ...
    const requiredFields =
      formStructure.fields?.filter((f: any) => f.required)?.length || 0;
    const hasValidation =
      formStructure.fields?.some((f: any) => f.validation) || false;
    ...
    return {
      ...
      analysis: {
        completeness: {
          ...
          missingElements:
            fieldCount < 5
              ? ['Consider adding more fields for comprehensive data capture']
              : [],
          recommendations:
            fieldCount < 3
              ? ['Add contact information fields', 'Include project timeline']
              : [],
        },
        usability: {
          score: usabilityScore,
          issues:
            requiredFields > fieldCount * 0.6
              ? ['Too many required fields may reduce completion rate']
              : [],
          improvements: [
            'Consider logical field grouping',
