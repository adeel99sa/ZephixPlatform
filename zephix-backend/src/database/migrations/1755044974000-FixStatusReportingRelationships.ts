...
export class FixStatusReportingRelationships1755044974000
  implements MigrationInterface
{
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    for (const column of duplicateColumns) {
      try {
        await queryRunner.query(
          `ALTER TABLE "status_reports" DROP COLUMN IF EXISTS "${column}"`,
        );
    ...
    const projectIdExists = await queryRunner.hasColumn(
      'status_reports',
      'projectId',
    );
    if (!projectIdExists) {
    ...
    const organizationIdExists = await queryRunner.hasColumn(
      'status_reports',
      'organizationId',
    );
    ...
    console.log(
      '🎉 Status reporting relationship fixes completed successfully!',
    );
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    console.log(
      '⚠️ Rollback completed - duplicate columns not restored to prevent conflicts',
    );
  }
}
