...
async function runWorkspaceMigrations() {
  const client = new Client({
    host: 'roundhouse.railway.internal',
    port: 5432,
    user: 'postgres',
  ...
  try {
    ...
    console.log(`Found ${checkWorkspaces.rows.length} existing hierarchy columns`);

    if (checkWorkspaces.rows.length < 3) {
      ...
      await client.query(`
        ALTER TABLE workspaces 
        ADD COLUMN IF NOT EXISTS workspace_type VARCHAR(50) DEFAULT 'standard';
      `);
    ...
    const testWorkspace = await client.query(`
      INSERT INTO workspaces (name, description, organization_id, parent_workspace_id, workspace_type, hierarchy_level)
      VALUES ('Test Workspace', 'Test hierarchy workspace', (SELECT id FROM organizations LIMIT 1), NULL, 'standard', 0)
      RETURNING id, name, parent_workspace_id, workspace_type, hierarchy_level
    `);
    
    console.log('✅ Test workspace created:', testWorkspace.rows[0]);

    console.log('\n🎉 All workspace migrations completed successfully!');
